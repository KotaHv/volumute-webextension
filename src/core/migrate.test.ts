...
describe('migration chain', () => {
  it('migrates legacy v1 mute entries (ts -> lastUsed, created falls back to lastUsed)', () => {
  ...
  it('migrates legacy v1 volume entries (v -> multiplier, t -> lastUsed)', () => {
    ...
    expect(e?.created).toBe(500);
  });

  it('fills missing created from lastUsed without touching the rest', () => {
    const map = { 'a.com': { enabled: true, lastUsed: 42, deviceId: 'x' } } as unknown as MuteMap;
    const out = migrateMap(MUTE_MIGRATIONS, map, 1, 2);
    const e = out?.['a.com'];
    expect(e?.created).toBe(42);
    expect(e?.lastUsed).toBe(42);
    expect(e?.enabled).toBe(true);
  });
