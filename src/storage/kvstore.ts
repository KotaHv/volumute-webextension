...
export class KVStore<T extends TimedEntry> {
  ...
  constructor(
    ...
    private readonly merge: (cache: EntryMap<T>, fresh: EntryMap<T> | undefined) => EntryMap<T> = (c, f) => ({ ...f, ...c }),
  ) {}
  ...
  async init(): Promise<void> {
    ...
    this.cache = mergeFresh(this.merge, {}, (stored[this.key] ?? {}) as EntryMap<T> | undefined)
    this.loaded = true
  ...
  async reload(): Promise<void> {
    ...
    const fresh = ((await browser.storage[this.area].get(this.key))[this.key] ?? {}) as EntryMap<T> | undefined
    this.cache = mergeFresh(this.merge, this.cache, fresh)
    this.emit()
  }
  ...
  async update(fn: (cache: EntryMap<T>) => EntryMap<T>, opts: { immediate?: boolean } = {}): Promise<void> {
  ...
  async touchEntry(entryKey: string): Promise<void> {
    ...
    this.writePromise = this.writePromise.then(async () => {
      try {
        ...
        if (!entry) return
        const now = Date.now()
        if ('t' in entry) (entry as T & { t: number }).t = now
        if ('ts' in entry) (entry as T & { ts: number }).ts = now
        await area.set({ [this.key]: map })
        this.cache = mergeFresh(this.merge, this.cache, map)
        this.emit()
      } catch (err) {
        console.warn(`[KVStore] touchEntry failed for ${this.area}:${this.key}`, err)
      }
    })
    return this.writePromise
  }
    ...
    this.writePromise = this.writePromise.then(async () => {
      ...
      for (;;) {
        ...
        if (attempts > MAX_WRITE_ATTEMPTS) {
          console.warn(`[KVStore] write to ${this.area}:${this.key} failed after ${attempts} attempts`)
          return
        }
        try {
          ...
          if (isQuotaError(err)) {
            ...
            if (!evicted) {
              console.error(`[KVStore] quota exceeded and nothing left to evict in ${this.key}`)
              return
            }
            value = evicted
            ...
            console.error(`[KVStore] write failed for ${this.area}:${this.key}`, err)
            return
          }
        }
      }
